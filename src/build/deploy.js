...
const Deploy = {
    doDeploy() {
        ...
        PrototypePackager.packageServer(require("./prototype-server-config"), __dirname + "/../..").then(() => {
            ...
            let deployDir = `//Users/quanle/Documents/Workon/Pure-React/sample-vcb-deploy`;
            extract(uploadedPath, {dir: deployDir}, function (err) {
                fs.unlink(uploadedPath);
                fs.unlink(deployDir + "/prototype-server.json");

                replaceInFile(`${deployDir}/index.html`, `<base href="/">`, `<base href="https://quanla.github.io/pure-react-sample-vcb-pages/">`);
            });
        });
